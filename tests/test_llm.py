...
from EvoScientist.llm.models import _MODEL_ENTRIES, _installed_codex_client_version


@pytest.fixture(autouse=True)
def _reset_codex_version_cache():
    """Keep the codex --version lru_cache from leaking host state across tests."""
...
class TestListModels:
    ...
    def test_includes_gpt_5_6_variants(self):
        """Current GPT-5.6 variants remain available in the model picker."""
...
class TestAutoConfig:
    ...
    @patch("EvoScientist.llm.models.init_chat_model")
    def test_openai_ccproxy_reasoning_effort_from_env(
        self, mock_init, mock_installed_version, monkeypatch
    ):
        """Configured reasoning effort reaches ccproxy's Responses API route."""
        mock_init.return_value = "mock_model"
        monkeypatch.setenv("OPENAI_BASE_URL", "http://127.0.0.1:8000/codex/v1")
        monkeypatch.setenv("OPENAI_API_KEY", "ccproxy-oauth")
        monkeypatch.setenv("EVOSCIENTIST_REASONING_EFFORT", "high")
    ...
    def test_openai_ccproxy_codex_client_version_env(self, mock_init, monkeypatch):
        """EVOSCIENTIST_CODEX_CLIENT_VERSION overrides the minimum version."""
        mock_init.return_value = "mock_model"
        monkeypatch.setenv("OPENAI_BASE_URL", "http://127.0.0.1:8000/codex/v1")
        monkeypatch.setenv("OPENAI_API_KEY", "ccproxy-oauth")
