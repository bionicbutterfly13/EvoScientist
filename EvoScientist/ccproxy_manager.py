...
# Cold ccproxy CLI startup (plugin init + Codex CLI detection) can exceed
# 30s while credentials are still valid; 30s caused intermittent auth
# false-negatives (see c562514). Keep parity with the health budget.
_CCPROXY_AUTH_TIMEOUT_SECONDS = 180
...
def check_ccproxy_auth(provider: str = "claude_api") -> tuple[bool, str]:
    ...
    try:
        ...
        result = subprocess.run(
            ...
            # invocation; cold startup can exceed 30s with valid credentials,
            # so short deadlines here fail launch before the proxy can warm.
